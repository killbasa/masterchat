...
import type { Color, TimedContinuation } from './interfaces/misc';
import type { YTAction, YTContinuationContents, YTEmojiRun, YTRun, YTSimpleTextContainer, YTText, YTTextRun } from './interfaces/yt/chat';
import type { FluffyBrowseEndpoint } from './interfaces/yt/context';

export type ColorFormat = 'hex' | 'rgb';

export type RunsToStringOptions = {
...
export function splitRunsByNewLines(runs: YTRun[]): YTRun[][] {
	return runs.reduce<YTRun[][]>((s, i) => ('text' in i && i.text === '\n' ? [...s, []] : (s[s.length - 1].push(i), s)), [[]]);
}
 ...
 */
export function stringify(payload: YTRun[] | YTText | string, runsToStringOptions?: RunsToStringOptions): string;
export function stringify(payload: undefined, runsToStringOptions?: RunsToStringOptions): undefined;
export function stringify(payload: YTRun[] | YTText | string | undefined, runsToStringOptions?: RunsToStringOptions): string | undefined {
	// undefined
...
export function simpleTextToString(payload: YTSimpleTextContainer, expand = false) {
...
export function groupBy<T, K extends keyof T, S extends Extract<T[K], string>>(lst: T[], key: K) {
	return lst.reduce<{ [k in S]: (T extends { [S in K]: k } ? T : never)[] }>((result, o) => {
		const index = o[key] as S;
		// eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
		if (!result[index]) result[index] = [];
		result[index].push(o as any);
		return result;
		// @ts-expect-error idk what to put
	}, {});
}
...
export function durationToSeconds(durationText: string): number {
	...
	const durationInt = parseInt(duration, 10) || 1;
	const multiplier = {
		year: 31536000,
		month: 2628000,
		week: 604800,
